...
{
    gp_Trsf tran;
    tran.SetValues(scale.x(), 0, 0, 0, 0, scale.y(), 0, 0, 0, 0, scale.z(), 0);
    BRepBuilderAPI_Transform transform(shape_, tran);
    shape_ = transform.Shape();
}
...
{
    gp_Trsf tran;
    tran.SetValues(transform(0, 0), transform(0, 1), transform(0, 2), transform(0, 3), transform(1, 0), transform(1, 1),
...
{
    gp_Trsf tran;
    tran.SetValues(transform.matrix()(0, 0), transform.matrix()(0, 1), transform.matrix()(0, 2),
                   transform.matrix()(0, 3), transform.matrix()(1, 0), transform.matrix()(1, 1),
                   transform.matrix()(1, 2), transform.matrix()(1, 3), transform.matrix()(2, 0),
                   transform.matrix()(2, 1), transform.matrix()(2, 2), transform.matrix()(2, 3));
    BRepBuilderAPI_Transform transformer(shape_, tran);
    shape_ = transformer.Shape();
}
...
std::pair<Eigen::MatrixXf, Eigen::MatrixXi> OcctModel::TriangleMesh() const
{
    ...
    for (TopExp_Explorer anExpSF(shape_, TopAbs_FACE); anExpSF.More(); anExpSF.Next())
    {
        ...
        for (Standard_Integer aTriIter = 1; aTriIter <= aTriangulation->NbTriangles(); ++aTriIter)
        {
            ...
            faces.push_back({anId[0] - 1 + aNodeOffset, anId[1] - 1 + aNodeOffset, anId[2] - 1 + aNodeOffset});
        }
    }
    Eigen::MatrixXf v;
    Eigen::MatrixXi f;
...
OcctModel OcctModel::CreateTorus(const float majorRadius, const float minorRadius, const int majorSegments,
...
OcctModel ThickSolid(const OcctModel& model, const std::vector<std::vector<Eigen::Vector3f>>& faces, float thickness)
{
    ...
    for (TopExp_Explorer anExpSF(model.shape_, TopAbs_FACE); anExpSF.More(); anExpSF.Next())
    {
        ...
        if (std::find_if(faces_waiter.begin(), faces_waiter.end(),
                         [&face, &faces_close](const auto& f) { return faces_close(f, face); }) != faces_waiter.end())
        {
            remove_shape.Append(face);
        }
    }
