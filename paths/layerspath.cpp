...
LayersPath::LayersPath(const std::function<void(std::string_view, std::string_view)>& callback) : callback_(callback)
{
}
...
void LayersPath::Save(const std::filesystem::path& path) const
{
    ...
    db | SQL::SQLCreateTable("layers", {{"id", "INTEGER PRIMARY KEY AUTOINCREMENT"},
    ...
    for (const auto& layerData : layers_)
    {
        ...
        ss << "}";
        db | SQL::SQLInsert("layers", {{"layer_config", layerData.layerConfig}, {"layer_data", ss.str()}});
    }
}
                      ...
                      const std::function<void(lua_State*)>& lua_reg) const
{
    ...
    *db += callback_;
    ...
    for (const auto& layerData : layers_)
    {
        ...
        lua_pushstring(L.get(), layerData.layerConfig.c_str());
        lua_setfield(L.get(), -2, "config");
        lua_newtable(L.get());
        int poly_idx = 1;
        for (const auto& poly : layerData.layer)
        {
            ...
            {
                lua_newtable(L.get());
                lua_pushnumber(L.get(), pt.x);
                lua_setfield(L.get(), -2, "x");
                lua_pushnumber(L.get(), pt.y);
...
void LayersPath::Save(const std::filesystem::path& path, const std::filesystem::path& script_file,
...
std::string LayersPath::ToString(const std::string_view script, const std::function<void(lua_State*)>& lua_reg) const
{
    if (script.empty())
        return ToString();
