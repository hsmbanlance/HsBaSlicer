...
const char* RLPointTypeToString(RLPointType t)
{
    ...
    case RLPointType::ProgramLStart:
        return "ProgramLStart";
    case RLPointType::ProgramStart:
        return "ProgramStart";
    case RLPointType::ProgramCStart:
        return "ProgramCStart";
...
RobotPath::RobotPath(RLType robotType, OutPoints3 startPoint, std::string startProgramFunc, std::string endProgramFunc)
    : robotType_{robotType}, points_{}, startPoint_{startPoint}, startProgramFunc_{startProgramFunc},
      endProgramFunc_{endProgramFunc}
{
}
...
std::string RobotPath::ToString(std::string_view script, const std::function<void(lua_State*)>& lua_reg) const
{
    // When script provided: prepend header that declares script is user-provided and robot type is ignored
    const std::string header =
        "// Script provided by user - robot type ignored, remove this line when using real robots\n";
    ...
    for (const auto& pt : points_)
    {
        ...
        lua_pushstring(L.get(), RLPointTypeToString(pt.type));
...
std::string RobotPath::GenerateAbbCode() const
{
    ...
    ss << "  PROC main()\n";
    // start point
    ss << "    MOVEJ " << std::fixed << std::setprecision(4) << "[" << startPoint_.x << "," << startPoint_.y << ","
       << startPoint_.z << ",0.0,0.0,0.0]"
    ...
    for (size_t i = 0; i < points_.size(); ++i)
    {
        ...
        case RLPointType::MoveJ:
            ...
            ss << " ,[" << pt.velocity << "," << ABB_MIN_SPEED << "," << ABB_MAX_SPEED << "," << ABB_HIGH_SPEED
               << "] ,z10 ,tooldata1\\Wobj=workjob1;\n";
            break;
        case RLPointType::MoveL:
            ss << "    MOVEL ";
            ss << "[" << pt.end.x << "," << pt.end.y << "," << pt.end.z << ",0.0,0.0,0.0]";
        ...
        case RLPointType::MoveC:
            ...
            ss << "[" << pt.middle.x << "," << pt.middle.y << "," << pt.middle.z << ",0.0,0.0,0.0], "
               ...
               << " ,[" << pt.velocity << "," << ABB_MIN_SPEED << "," << ABB_MAX_SPEED << "," << ABB_HIGH_SPEED
               << "] ,z10 ,tooldata1\\Wobj=workjob1;\n";
            break;
        case RLPointType::ProgramCStart:
        case RLPointType::ProgramC:
...
std::string RobotPath::GenerateKukaCode() const
{
    ...
    for (size_t i = 0; i < points_.size(); ++i)
    {
        ...
        {
            ss << "  CIRC {X " << pt.middle.x << ", Y " << pt.middle.y << ", Z " << pt.middle.z
               << ", A 0, B 0, C 0} {X " << pt.end.x << ", Y " << pt.end.y << ", Z " << pt.end.z
               << ", A 0, B 0, C 0} C_DIS ;";
        }
        else
        {
        ...
        if (pt.type == RLPointType::ProgramLStart || pt.type == RLPointType::ProgramStart ||
            pt.type == RLPointType::ProgramCStart || pt.type == RLPointType::ProgramL ||
            pt.type == RLPointType::ProgramC || pt.type == RLPointType::ProgramLEnd ||
            pt.type == RLPointType::ProgramCEnd)
        {
...
std::string RobotPath::GenerateFanucCode() const
{
    ...
    for (size_t i = 0; i < points_.size(); ++i)
    {
            ...
            pt.type == RLPointType::ProgramC || pt.type == RLPointType::ProgramCEnd)
        {
            ...
            ss << "  ARC P_VIA P_END 100% FINE ;  ! via=(" << pt.middle.x << "," << pt.middle.y << "," << pt.middle.z
               << ") end=(" << pt.end.x << "," << pt.end.y << "," << pt.end.z << ")";
        }
        else
        {
            ss << "  J P[" << i << "] 100% FINE ;";
...
std::string RobotPath::ToString(std::string_view script, std::string_view funcName,
                                const std::function<void(lua_State*)>& lua_reg) const
{
    auto L = MakeUniqueLuaState();
    if (!L)
...
std::string RobotPath::ToString(const std::filesystem::path& script_file, const std::string_view funcName,
                                const std::function<void(lua_State*)>& lua_reg) const
{
...
void RobotPath::Save(const std::filesystem::path& path, std::string_view script, std::string_view funcName,
                     const std::function<void(lua_State*)>& lua_reg) const
{
...
void RobotPath::Save(const std::filesystem::path& path, const std::filesystem::path& script_file,
                     std::string_view funcName, const std::function<void(lua_State*)>& lua_reg) const
{
