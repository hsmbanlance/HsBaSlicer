...
## 更新摘要
**所做更改**
- 将CMake最低版本要求从3.21提升至3.28以启用高级C++20特性
- 新增HSBA_SLICER_MODULE选项控制C++20模块包装层构建
- 改进跨平台兼容性，特别增强iOS平台支持
- 完善安装配置以支持静态和共享库构建，包含新的ModuleHsBaSlicer目标
- 新增C++20模块检测机制和编译器版本检查
...
本指南旨在为 HsBaSlicer 项目提供全面的构建与部署说明。项目采用 CMake 作为构建系统，vcpkg 作为第三方依赖管理工具，支持跨平台构建（Windows、Linux、macOS、Android 和 iOS）。**重大更新**：项目现已升级至CMake 3.28+，支持C++20模块功能，并引入了全新的ModuleHsBaSlicer包装层，提供更现代的类风格API接口。
...
HsBaSlicer 项目采用模块化设计，将功能划分为多个子目录，每个子目录包含其自身的 `CMakeLists.txt` 文件。这种结构提高了代码的可维护性和可重用性。**新增**：现在包含独立的ModuleHsBaSlicer模块，提供C++20模块包装层。
...
### 子目录组织
通过 `add_subdirectory` 命令，顶层 CMakeLists.txt 按顺序包含所有子模块。**重要更新**：新增了 `preprocess` 和 `support` 子目录的包含，这两个新模块提供了模型预处理和支撑生成的核心功能。**新增**：现在包含条件性的 `ModuleHsBaSlicer` 子目录，仅在启用模块支持时构建。
...
**Section sources**
- [CMakeLists.txt:1-597](file://CMakeLists.txt#L1-L597)

## CMake安装系统详解

**重大更新**：项目现已实现完整的CMake安装系统，支持标准化的库安装和包发现，并包含新的ModuleHsBaSlicer目标。
...
**重大更新**：项目现在支持多种打包和分发方式，包括标准CMake安装和自定义部署脚本，并包含新的ModuleHsBaSlicer目标支持。
...
**重要更新**：CADModel相关的头文件和库仅在桌面平台上安装，移动端和游戏主机平台不会包含这些文件。**新增**：ModuleHsBaSlicer的C++20模块文件会安装在 `include/HsBaSlicer/modules/` 目录下。
