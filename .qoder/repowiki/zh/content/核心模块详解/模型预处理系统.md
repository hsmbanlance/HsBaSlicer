...
## 更新摘要
**所做更改**   
- 改进了模型信息提取精度，特别是包围盒计算和体积计算算法
- 优化了IglModel的体积计算实现，使用更精确的三角网格体积算法
- 增强了OcctModel的包围盒计算，通过遍历所有顶点确保准确性
- 提升了材料估算的精确度，为3D打印提供更准确的耗材用量预测
...
**最新更新** 系统已改进模型信息提取精度，特别是包围盒计算和体积计算算法，为3D打印提供更准确的耗材用量预测。
...
graph TB
subgraph "预处理"
A["model_preprocess.hpp/.cpp"]
B["ModelLoader.hpp/.cpp"]
C["IModel.hpp"]
end
subgraph "网格模型"
D["IglModel.hpp/.cpp"]
...
subgraph "支撑"
K["fdm_support.hpp/.cpp"]
L["support/FdmSupport.hpp"]
M["support/ISupport.hpp"]
N["support/SupportConfig.hpp"]
end
subgraph "填充"
O["polygon_fill.hpp/.cpp"]
P["2D/PolygonFill.hpp"]
Q["2D/IntPolygon.hpp"]
R["2D/FloatPolygons.hpp"]
end
subgraph "路径"
S["path_generator.hpp/.cpp"]
T["paths/pointspath.hpp"]
end
subgraph "协程"
U["coroutine.hpp"]
end
subgraph "DLL入口"
V["fdm_pipeline.h/.cpp"]
end
A --> B
B --> C
C --> D
...
## 核心组件
- 模型抽象接口 IModel：定义统一的加载/保存、几何变换、包围盒/体积查询、三角网格提取等能力，屏蔽不同内核（网格/BRep）差异。
- 模型加载器 ModelLoader：基于扩展名自动选择内核（网格或BRep），并通过命名对象池管理模型生命周期；在启用CGAL时提供厚壳与布尔运算等高级能力。
- 预处理封装 model_preprocess：对外暴露加载、获取、变换、信息查询与移除等简洁API，内部使用线程局部存储的ModelLoader实例。
- **增强的网格模型 IglModel**：实现了优化的体积计算算法，使用IGL库的精确三角网格体积计算方法，以及高效的包围盒计算。
- **增强的BRep模型 OcctModel**：提供了精确的体积计算（基于OCCT的几何属性计算）和改进的包围盒计算（通过遍历所有顶点）。
...
- [LibHsBaSlicer/Preprocess/model_preprocess.hpp:15-85](file://LibHsBaSlicer/Preprocess/model_preprocess.hpp#L15-L85)
...
- [base/coroutine.hpp:36-800](file://base/coroutine.hpp#L36-L800)
- [DllHsBaSlicer/fdm_pipeline.h:1-147](file://DllHsBaSlicer/fdm_pipeline.h#L1-L147)
...
participant Pre as "预处理(model_preprocess)"
participant Loader as "ModelLoader"
participant Mesh as "IglModel(优化)"
participant BRep as "OcctModel(优化)"
...
Loader->>Mesh : "创建网格模型"
Loader->>BRep : "创建BRep模型"
Mesh-->>Loader : "IModel*"
BRep-->>Loader : "IModel*"
Loader-->>Pre : "IModel*"
Pre-->>DLL : "IModel*"
DLL->>Pre : "GetModelInfo(name)"
Pre->>Mesh : "BoundingBox + Volume(优化)"
Pre->>BRep : "BoundingBox + Volume(优化)"
Mesh-->>Pre : "精确包围盒+体积"
BRep-->>Pre : "精确包围盒+体积"
...
loop 逐层填充
DLL->>Fill : "FillWithBorder(int_polys, spacing, border_count, mode, angle)"
...
- [LibHsBaSlicer/Preprocess/model_preprocess.cpp:17-78](file://LibHsBaSlicer/Preprocess/model_preprocess.cpp#L17-L78)
...
### 模型抽象与加载（IModel + ModelLoader + model_preprocess）
- IModel定义了跨内核的统一能力，包括加载/保存、平移/旋转/缩放/仿射变换、包围盒与体积、三角网格提取。
- ModelLoader根据文件扩展名自动选择网格内核（STL/OBJ/PLY/OFF）或BRep内核（STEP/IGES/VRML/BREP），并将模型放入命名对象池统一管理；在启用CGAL时提供厚壳与布尔运算。
- model_preprocess对外暴露LoadModel/GetModel/Translate/Rotate/Scale/GetModelInfo/RemoveModel等API，内部使用线程局部ModelLoader实例，保证多线程安全与资源隔离。
...
ModelPreprocess --> ModelLoader : "委托"
ModelLoader --> IModel : "持有"
IModel <|-- IglModel : "继承"
IModel <|-- OcctModel : "继承"
```
...
- [preprocess/ModelLoader.hpp:26-127](file://preprocess/ModelLoader.hpp#L26-L127)
- [LibHsBaSlicer/Preprocess/model_preprocess.hpp:15-85](file://LibHsBaSlicer/Preprocess/model_preprocess.hpp#L15-L85)
- [meshmodel/IglModel.hpp:14-42](file://meshmodel/IglModel.hpp#L14-L42)
- [cadmodel/OcctModel.hpp:18-49](file://cadmodel/OcctModel.hpp#L18-L49)
...
```mermaid
flowchart TD
Start(["开始"]) --> GetVertices["获取所有顶点数据"]
GetVertices --> CalcMin["计算各维度最小值<br/>min = vertices_.colwise().minCoeff()"]
GetVertices --> CalcMax["计算各维度最大值<br/>max = vertices_.colwise().maxCoeff()"]
CalcMin --> ReturnBB["返回包围盒"]
CalcMax --> ReturnBB
ReturnBB --> End(["结束"])
```

**图示来源**
- [meshmodel/IglModel.cpp:140-144](file://meshmodel/IglModel.cpp#L140-L144)
...
```mermaid
flowchart TD
Start(["开始"]) --> InitBounds["初始化边界<br/>min=FLT_MAX, max=-FLT_MAX"]
InitBounds --> LoopVertices["遍历所有顶点<br/>for(TopExp_Explorer exp(shape_, TopAbs_VERTEX))"]
LoopVertices --> GetPoint["获取顶点坐标<br/>gp_Pnt pnt = tool.Pnt(vertex)"]
GetPoint --> UpdateMin["更新最小值<br/>min.x/y/z = std::min(...)"]
UpdateMin --> UpdateMax["更新最大值<br/>max.x/y/z = std::max(...)"]
UpdateMax --> NextVertex{"还有下一个顶点?"}
NextVertex --> |是| LoopVertices
NextVertex --> |否| ReturnBB["返回精确包围盒"]
ReturnBB --> End(["结束"])
```

**图示来源**
- [cadmodel/OcctModel.cpp:326-346](file://cadmodel/OcctModel.cpp#L326-L346)
...
章节来源
- [base/coroutine.hpp:36-800](file://base/coroutine.hpp#L36-L800)
- [DllHsBaSlicer/fdm_pipeline.cpp:324-348](file://DllHsBaSlicer/fdm_pipeline.cpp#L324-L348)
...
```mermaid
graph LR
Pre["预处理(model_preprocess)"] --> Loader["ModelLoader"]
Loader --> IModel["IModel"]
IModel --> IglModel["IglModel(优化)"]
IModel --> OcctModel["OcctModel(优化)"]
IglModel --> IGL["IGL库"]
OcctModel --> OCCT["OCCT库"]
...
**图示来源**
- [LibHsBaSlicer/Preprocess/model_preprocess.cpp:1-81](file://LibHsBaSlicer/Preprocess/model_preprocess.cpp#L1-L81)
...
## 结论
模型预处理系统通过清晰的接口抽象与模块化封装，实现了从模型加载、切片、支撑、填充到路径生成的完整FDM流水线。**最新的改进显著提升了模型信息提取的精度**，特别是包围盒计算和体积计算算法的优化，为3D打印提供了更准确的耗材用量预测。配合协程框架，既提供同步阻塞的易用性，也支持异步非阻塞的高吞吐场景。建议在工程实践中结合硬件能力与模型规模，合理配置并行度与参数，以获得最佳性能与质量平衡。
