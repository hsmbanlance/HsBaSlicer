...
## 更新摘要
**所做更改**   
- 新增了Graph算法测试套件，涵盖基础图操作、最短路径算法、最小生成树、连通分量、最大流等核心图论算法
- 增强了路径优化测试，包括区域路径优化和多边形模式的路径优化
- 添加了Lua脚本测试用例，支持动态路径优化策略
- 完善了多语言文档支持，包括完整的英文和中文文档结构
...
**更新** 本次更新重点增强了图算法测试和路径优化测试能力，新增了全面的Graph算法测试套件和智能路径优化测试，同时提供了完整的英文和中文文档体系，包括各模块的详细使用说明和架构图。
...
- **新增** 图算法测试套件
  - 涵盖基础图操作、遍历算法、最短路径算法、最小生成树、连通分量、拓扑排序、最大流等完整图论算法测试。
- **新增** 路径优化测试
  - 支持区域路径优化和多边形模式的路径优化，包含Lua脚本集成测试。
- **新增** 多语言文档支持
...
- [tests/CMakeLists.txt:156-185](file://tests/CMakeLists.txt#L156-L185)
- [tests/CMakeLists.txt:697-723](file://tests/CMakeLists.txt#L697-L723)
...
E --> N["graph_test<br/>图算法测试"]
E --> O["polygon_fill_test<br/>路径优化测试"]
N --> P["utils/Graph.hpp<br/>图算法实现"]
O --> Q["utils/AreaGraph.hpp<br/>区域图算法"]
```
...
- [tests/CMakeLists.txt:727-731](file://tests/CMakeLists.txt#L727-L731)
- [tests/CMakeLists.txt:755-759](file://tests/CMakeLists.txt#L755-L759)
- [tests/CMakeLists.txt:782-786](file://tests/CMakeLists.txt#L782-L786)
...
### 图算法测试套件（新增）
**新增** 项目现已包含完整的图算法测试套件，覆盖核心图论算法：

- **基础图操作测试**
  - 顶点添加与查询、边权重设置、邻居节点访问、重复顶点检测等基础功能验证
- **遍历算法测试**
  - BFS广度优先搜索和DFS深度优先搜索的顺序验证
- **最短路径算法测试**
  - Dijkstra算法、A*算法、Bellman-Ford算法的正确性验证
- **最小生成树测试**
  - Prim算法和Kruskal算法的生成树验证
- **连通性与拓扑测试**
  - 连通分量、强连通分量、拓扑排序算法测试
- **最大流算法测试**
  - Ford-Fulkerson最大流算法验证
- **遗传算法TSP测试**
  - 旅行商问题的遗传算法求解测试
- **区域图测试**
  - 复杂区域图的出入口管理和路径规划测试
...
```mermaid
flowchart TD
Start(["图算法测试套件"]) --> Base["基础图操作<br/>顶点/边/邻居操作"]
Start --> Traversal["遍历算法<br/>BFS/DFS顺序验证"]
Start --> ShortestPath["最短路径算法<br/>Dijkstra/A*/Bellman-Ford"]
Start --> MST["最小生成树<br/>Prim/Kruskal算法"]
Start --> Connectivity["连通性分析<br/>连通分量/拓扑排序"]
Start --> MaxFlow["最大流算法<br/>Ford-Fulkerson"]
Start --> GeneticTSP["遗传算法TSP<br/>旅行商问题求解"]
Start --> AreaGraph["区域图测试<br/>出入口管理"]
Base --> Compile["编译期验证"]
Traversal --> Compile
ShortestPath --> Compile
MST --> Compile
...
**图表来源**
- [tests/Graph/graph_test.cpp:57-155](file://tests/Graph/graph_test.cpp#L57-L155)
- [tests/Graph/graph_test.cpp:197-246](file://tests/Graph/graph_test.cpp#L197-L246)
- [tests/Graph/graph_test.cpp:251-341](file://tests/Graph/graph_test.cpp#L251-L341)
- [tests/Graph/graph_test.cpp:346-394](file://tests/Graph/graph_test.cpp#L346-L394)
- [tests/Graph/graph_test.cpp:399-469](file://tests/Graph/graph_test.cpp#L399-L469)
- [tests/Graph/graph_test.cpp:474-499](file://tests/Graph/graph_test.cpp#L474-L499)
- [tests/Graph/graph_test.cpp:504-570](file://tests/Graph/graph_test.cpp#L504-L570)
- [tests/Graph/graph_test.cpp:575-730](file://tests/Graph/graph_test.cpp#L575-L730)

章节来源
- [tests/Graph/graph_test.cpp:1-782](file://tests/Graph/graph_test.cpp#L1-L782)
- [tests/CMakeLists.txt:156-185](file://tests/CMakeLists.txt#L156-L185)
- [utils/Graph.hpp:1-200](file://utils/Graph.hpp#L1-L200)
- [utils/AreaGraph.hpp:1-200](file://utils/AreaGraph.hpp#L1-L200)

### 路径优化测试（新增）
**新增** 项目包含了先进的路径优化测试，支持多种优化模式：

- **区域路径优化测试**
  - 测试RegionPathOptimizer的区域聚簇和路径反转功能
  - 验证不同区域间的优化顺序和路径连续性
- **多边形模式路径优化**
  - 支持填充前多边形区域的优化
  - 验证多边形起点旋转和方向保持
...
```mermaid
classDiagram
class PathOptimizerTests {
+optimizer_cluster_order()
+optimizer_single_region_and_path_reversal()
+optimizer_manual_route()
+lua_script_embedded_optimize()
+polygon_mode_cluster_order()
+polygon_mode_entry_rotation()
+lua_polygon_mode_embedded_optimize()
}
...
class LuaOptimization {
+LuaOptimizeRegionPaths()
+LuaOptimizeRegionPolygons()
+optimize_paths.lua
}
PathOptimizerTests --> RegionPathOptimizer
PathOptimizerTests --> LuaOptimization
```

**图表来源**
- [tests/PolygonFill/polygon_fill_test.cpp:229-453](file://tests/PolygonFill/polygon_fill_test.cpp#L229-L453)
- [tests/PolygonFill/optimize_paths.lua:1-34](file://tests/PolygonFill/optimize_paths.lua#L1-L34)

章节来源
- [tests/PolygonFill/polygon_fill_test.cpp:1-453](file://tests/PolygonFill/polygon_fill_test.cpp#L1-L453)
- [tests/PolygonFill/optimize_paths.lua:1-34](file://tests/PolygonFill/optimize_paths.lua#L1-L34)
- [tests/CMakeLists.txt:697-723](file://tests/CMakeLists.txt#L697-L723)
...
Boost --> T4["graph_test<br/>图算法测试"]
Boost --> T5["polygon_fill_test<br/>路径优化测试"]
HBase["HsBaSlicerBase"] --> T1
HBase --> T2
HBase --> T4
HFileOp["HsBaSlicerFileOperator"] --> T3
HEigen["Eigen3::Eigen"] --> T3
H2D["HsBaSlicer2D"] --> T5
HLua["LibHsBaSlicer"] --> T5
...
- **新增** 图算法测试性能优化
  - Graph测试在MSVC平台启用/bigobj选项以解决模板实例化限制
  - 图算法测试覆盖大量算法实现，需要合理的内存和计算资源
- **新增** 路径优化测试性能考虑
  - 区域路径优化涉及复杂的聚类算法和TSP求解，需要足够的计算资源
  - Lua脚本优化策略支持灵活配置，可根据实际需求调整优化强度
- **新增** 文档生成优化
...
- [tests/CMakeLists.txt:156-162](file://tests/CMakeLists.txt#L156-L162)
- [tests/CMakeLists.txt:697-723](file://tests/CMakeLists.txt#L697-L723)
- [static_tests/README.md:1-4](file://static_tests/README.md#L1-L4)
- [docs/README.md:1-164](file://docs/README.md#L1-L164)
...
- **新增** 图算法测试编译问题
  - 现象：MSVC平台下图算法测试编译失败，提示段限制错误
  - 处理：确认已启用/bigobj编译选项；检查Boost.Graph模板实例化数量
- **新增** 路径优化测试Lua脚本问题
  - 现象：路径优化测试中Lua脚本执行失败
  - 处理：确认optimize_paths.lua文件路径正确；检查PathOptimize对象是否正确初始化
- **新增** 多语言文档访问问题
  - 现象：无法访问特定语言的文档页面
...
## 结论
本项目围绕 Boost.Test 与 CTest 构建了完善的运行时测试体系，辅以 static_tests 在编译期保障模板与常量表达式的正确性；同时通过 Doxygen 提供 API 文档生成能力，并以 CMakePresets 统一多平台构建体验。**本次更新显著增强了图算法测试和路径优化测试能力**，新增了全面的Graph算法测试套件覆盖基础图操作、遍历算法、最短路径、最小生成树、连通性分析、最大流等核心图论算法，以及智能路径优化测试支持区域聚簇、多边形模式优化和Lua脚本集成。同时提供了完整的英文和中文文档体系，包括各模块的详细使用说明和架构图，大大提升了项目的可维护性和用户体验。整体设计清晰、可扩展性强，适合持续集成与团队协作。
...
- 常用命令
  ...
  - 运行图算法测试：ctest -R graph_test
  - 运行路径优化测试：ctest -R polygon_fill_test
...
- **新增** 图算法测试用例
  - 基础图操作：tests/Graph/graph_test.cpp 中的 BaseGraphOperations 测试套件
  - 最短路径算法：tests/Graph/graph_test.cpp 中的 ShortestPathAlgorithms 测试套件
  - 路径优化策略：tests/PolygonFill/optimize_paths.lua 中的优化脚本
  - 区域路径优化：tests/PolygonFill/polygon_fill_test.cpp 中的 PathOptimizerTests 测试套件
