...
static void OnPipelineComplete(HsBaFdmPipelineResult_t result, void* user_data)
{
    ...
    if (result.success)
    {
        LogMsg(std::format("[异步] 切片完成! 层数: {}, 耗时: {:.2f} 秒",
                           result.total_layers, result.elapsed_seconds));
        *flag = 1;
    }
    else
    {
        LogMsg(std::format("[异步] 切片失败: {}",
                           result.error_message ? result.error_message : "未知错误"));
        *flag = -1;
    }
...
static int RunAsyncPipeline()
{
    ...
    // 异步启动（非阻塞）
    HsBaRunFdmPipelineAsync(&cfg, OnProgress, nullptr,
                            OnPipelineComplete, &done_flag);
