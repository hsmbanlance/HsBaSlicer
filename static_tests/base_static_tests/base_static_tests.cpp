...
void AnyVisitTests()
{
	...
	HsBa::Slicer::Utils::Visit<int, double>([](auto&& arg)
	{
		return static_cast<double>(arg + 1);
	}, a);

	std::any b = 2.0;

	HsBa::Slicer::Utils::Visit<int, double>(HsBa::Slicer::Utils::Overloaded{
		[](int item, int, double)
		{
...
void TupleEachTests()
{
	...
	HsBa::Slicer::Utils::TupleEach([](auto&& item)
	{
		using T = std::decay_t<decltype(item)>;
		static_assert(std::is_same_v<T, int> || std::is_same_v<T, double> || std::is_same_v<T, const char*>);
	}, tuple);

	HsBa::Slicer::Utils::TupleEach([](auto&& item, int)
	{
		using T = std::decay_t<decltype(item)>;
		static_assert(std::is_same_v<T, int> || std::is_same_v<T, double> || std::is_same_v<T, const char*>);
	}, tuple, 1);

	HsBa::Slicer::Utils::TupleEach([](auto&& item, int, double)
	{
		using T = std::decay_t<decltype(item)>;
		static_assert(std::is_same_v<T, int> || std::is_same_v<T, double> || std::is_same_v<T, const char*>);
		return 0;
	...
	HsBa::Slicer::Utils::TupleEach(HsBa::Slicer::Utils::Overloaded{
		[](int item, int, double)
		{
			static_assert(std::is_same_v<std::decay_t<decltype(item)>, int>);
			return 0;
...
HsBa::Slicer::Utils::CustomAllocatorTask<int, HsBa::Slicer::Utils::AsyncExecutor, std::allocator<int>> CustomAllocatorCoroutineTests()
{
	co_return 42;
}

HsBa::Slicer::Utils::CustomAllocatorTask<int, HsBa::Slicer::Utils::AsyncExecutor, std::allocator<int>> CustomAllocatorCoroutineTestsException()
{
	throw std::runtime_error("test exception");
	co_return 42;
}
...
void AnyObjectTests()
{
	...
	obj.Invoke("TestFunc", {});
	obj.ForeachField([](std::string_view name, HsBa::Slicer::Utils::AnyObject field)
	{
