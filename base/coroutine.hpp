...
{
public:
    inline void execute(std::function<void()>&& func) override { func(); }
};
...
{
public:
    inline void execute(std::function<void()>&& func) override { auto future = std::async(std::launch::async, func); }
};
template <typename T, typename Executor = AsyncExecutor>
    requires((std::movable<T> && std::default_initializable<T>) || std::is_void_v<T>) &&
    std::derived_from<Executor, IExecutor> class Task;
...
template <typename T, typename Executor>
    requires(std::movable<T> || std::is_void_v<T>) && std::derived_from<Executor, IExecutor> class TaskAwaiter
{
...
requires std::default_initializable<T>&& std::movable<T>&& std::derived_from<Executor, IExecutor>&&
    TAllocator<T, Allocator>&& NoStateAllocator<Allocator> class CustomAllocatorTask;
...
requires std::movable<T>&& std::derived_from<Executor, IExecutor>&& TAllocator<T, Allocator>&&
    NoStateAllocator<Allocator>&& NoStateAllocator<Allocator> class CustomAllocatorTaskAwaiter
{
    friend class CustomAllocatorTask<T, Executor, Allocator>;

public:
    inline explicit CustomAllocatorTaskAwaiter(Executor executor,
                                               CustomAllocatorTask<T, Executor, Allocator>&& task) noexcept
        : executor_(std::move(executor)), task_(std::move(task))
    {
    }
    CustomAllocatorTaskAwaiter(CustomAllocatorTaskAwaiter&& o) noexcept
        : executor_(std::move(o.executor_)), task_(std::move(o.task_))
    {
    }
...
{
public:
    inline explicit DispatchAwaiter(IExecutor* executor) noexcept : executor_(executor) {}
...
public:
    ...
    struct promise_type
    {
        ...
        template <typename R, typename Exe>
            ...
            std::derived_from<Exe, IExecutor> inline TaskAwaiter<R, Exe> await_transform(Task<R, Exe>&& task)
        {
            return TaskAwaiter<R, Exe>(std::move(executor_), std::move(task));
        }
        template <typename R, typename Exe, typename Allo>
        requires std::default_initializable<R>&& std::movable<R>&& std::derived_from<Exe, IExecutor>&&
            TAllocator<R, Allo> inline CustomAllocatorTaskAwaiter<R, Exe, Allo>
            await_transform(CustomAllocatorTask<R, Exe, Allo>&& task)
        {
    ...
    inline T get_result() { return handle_.promise().get_result(); }
...
public:
    ...
    inline void get_result() { handle_.promise().get_result(); }
...
public:
    ...
    struct promise_type
    {
        ...
        {
            return CustomAllocatorTask<T, Executor, Allocator>{
    ...
    inline void static SetAllocator(const Allocator& allocator) { promise_type::alloc = allocator; }
...
public:
    struct promise_type
    {
        ...
        inline return_type get_return_object() { return Generator{Handle::from_promise(*this)}; }
        inline static std::suspend_always initial_suspend() noexcept { return {}; }
        inline static std::suspend_always final_suspend() noexcept { return {}; }
    ...
    inline explicit Generator(const Handle coroutine) : coroutine_{coroutine} {}
    ...
    inline Generator(Generator&& other) noexcept : coroutine_{other.coroutine_} { other.coroutine_ = {}; }
    ...
    public:
        ...
        inline const T& operator*() const { return *coroutine_.promise().current_value; }
        inline bool operator==(std::default_sentinel_t) const { return !coroutine_ || coroutine_.done(); }

        inline explicit iterator(const Handle coroutine) : coroutine_{coroutine} {}
    ...
    inline static void SetOnCancel(std::function<void()> onCancel) { Generator::onCancel = onCancel; }
...
requires TAllocator<T, Allocator>&& NoStateAllocator<Allocator> class CustomAllocatorGenerator
{
public:
    using allocator_type = Allocator;
    struct promise_type
    ...
    inline explicit CustomAllocatorGenerator(const Handle coroutine) : coroutine_{coroutine} {}
    ...
    inline CustomAllocatorGenerator(CustomAllocatorGenerator&& other) noexcept : coroutine_{other.coroutine_}
    {
        other.coroutine_ = {};
    }
    ...
    inline static void SetOnCancel(std::function<void()> onCancel) { CustomAllocatorGenerator::onCancel = onCancel; }
