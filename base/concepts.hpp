...
concept CStyleString =
    std::same_as<T, const char*> || std::same_as<T, const wchar_t*> || std::same_as<T, const char16_t*> ||
...
template <typename T>
concept StringView =
    std::same_as<T, std::string_view> || std::same_as<T, std::wstring_view> || std::same_as<T, std::u16string_view> ||
    std::same_as<T, std::u32string_view> || std::same_as<T, std::u8string_view>;
