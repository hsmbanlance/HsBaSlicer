...
FullTopoModel::FullTopoModel(const IModel& model, bool use_normals)
{
    ...
    for (int i = 0; i != f.rows(); ++i)
    {
        ...
        // edge 0 1
        auto it01 = std::find_if(edges_.begin(), edges_.end(),
                                 [&face, &v0, &v1, &same_edge](const Edge& e) { return same_edge(e, v0, v1); });
        if (it01 != edges_.end())
        {
        ...
        // edge 1 2
        auto it12 = std::find_if(edges_.begin(), edges_.end(),
                                 [&face, &v1, &v2, &same_edge](const Edge& e) { return same_edge(e, v1, v2); });
        if (it12 != edges_.end())
        {
        ...
        // edge 2 0
        auto it20 = std::find_if(edges_.begin(), edges_.end(),
                                 [&face, &v2, &v0, &same_edge](const Edge& e) { return same_edge(e, v2, v0); });
        if (it20 != edges_.end())
        {
...
bool FullTopoModel::Intersetion(const Eigen::Vector3f& v1, const Eigen::Vector3f& v2, const float height,
...
Polygons FullTopoModel::Slice(const float height) const
{
    ...
    for (const auto& f : faces_)
    {
        ...
        Eigen::Vector3f p;
        if (Intersetion(v0, v1, height, p))
            inters.push_back(p);
        if (Intersetion(v1, v2, height, p))
            inters.push_back(p);
        if (Intersetion(v2, v0, height, p))
            inters.push_back(p);
