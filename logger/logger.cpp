...
HSBA_SLICER_LOG_API LoggerSingletone::LoggerSingletone(LoggerSingletone::Private)
{
    ...
    boost::log::formatter log_format =
        (boost::log::expressions::stream << boost::log::expressions::format_date_time<boost::posix_time::ptime>(
                                                "TimeStamp", log_datatime_format_)
    ...
    if (use_log_file_)
    {
        auto file_log = boost::log::add_file_log(
            boost::log::keywords::file_name = log_path_, boost::log::keywords::rotation_size = SIZE_50MB,
...
{
    std::call_once(instance_flag_, [&]() { instance_ = CreateInstance(); });
    return instance_;
}

HSBA_SLICER_LOG_API void LoggerSingletone::Log(std::string_view message, const int log_lv,
                                               const std::source_location& location)
{
    if (!instance_)
    {
        std::call_once(instance_flag_, [&]() { instance_ = std::make_shared<LoggerSingletone>(Private{}); });
    }
    switch (log_lv)
    {
    case 0:
    {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_VERBOSE, "HsBaSlicer", "%s%s", GetSourceLocation(location).c_str(),
                            std::string(message).c_str());
#elif defined(TARGET_OS_IOS) && TARGET_OS_IOS
        os_log_with_type(instance_->log_handle_, GetiOSLogType(log_lv), "%s%s", GetSourceLocation(location).c_str(),
                         std::string(message).c_str());
#else
    ...
    {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_DEBUG, "HsBaSlicer", "%s%s", GetSourceLocation(location).c_str(),
                            std::string(message).c_str());
#elif defined(TARGET_OS_IOS) && TARGET_OS_IOS
        os_log_with_type(instance_->log_handle_, GetiOSLogType(log_lv), "%s%s", GetSourceLocation(location).c_str(),
                         std::string(message).c_str());
    ...
    {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_INFO, "HsBaSlicer", "%s%s", GetSourceLocation(location).c_str(),
                            std::string(message).c_str());
#elif defined(TARGET_OS_IOS) && TARGET_OS_IOS
        os_log_with_type(instance_->log_handle_, GetiOSLogType(log_lv), "%s%s", GetSourceLocation(location).c_str(),
                         std::string(message).c_str());
    ...
    {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_WARN, "HsBaSlicer", "%s%s", GetSourceLocation(location).c_str(),
                            std::string(message).c_str());
#elif defined(TARGET_OS_IOS) && TARGET_OS_IOS
        os_log_with_type(instance_->log_handle_, GetiOSLogType(log_lv), "%s%s", GetSourceLocation(location).c_str(),
                         std::string(message).c_str());
    ...
    {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_ERROR, "HsBaSlicer", "%s%s", GetSourceLocation(location).c_str(),
                            std::string(message).c_str());
#elif defined(TARGET_OS_IOS) && TARGET_OS_IOS
        os_log_with_type(instance_->log_handle_, GetiOSLogType(log_lv), "%s%s", GetSourceLocation(location).c_str(),
                         std::string(message).c_str());
    ...
    {
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_FATAL, "HsBaSlicer", "%s%s", GetSourceLocation(location).c_str(),
                            std::string(message).c_str());
#elif defined(TARGET_OS_IOS) && TARGET_OS_IOS
        os_log_with_type(instance_->log_handle_, GetiOSLogType(log_lv), "%s%s", GetSourceLocation(location).c_str(),
                         std::string(message).c_str());
...
{
HSBA_SLICER_LOG_API LogState::LogState(const int log_lv, std::string_view message) : log_lv_{log_lv}, message_{message}
{
}
