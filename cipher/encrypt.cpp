...
std::vector<unsigned char> Encrypt::aes256_cbc_encrypt(const std::vector<unsigned char>& plaintext,
...
std::vector<unsigned char> Encrypt::aes256_cbc_decrypt(const std::vector<unsigned char>& cipher,
                                                       std::string_view password)
{
    unsigned char key[AES_KEY_SIZE];
    unsigned char iv[AES_IV_SIZE];
...
std::vector<unsigned char> Encrypt::aes256_ecb_encrypt(const std::vector<unsigned char>& plaintext,
                                                       std::string_view password)
{
    unsigned char key[AES_KEY_SIZE];
    unsigned char iv[AES_IV_SIZE];
...
std::vector<unsigned char> Encrypt::aes256_ecb_decrypt(const std::vector<unsigned char>& cipher,
                                                       std::string_view password)
{
    unsigned char key[AES_KEY_SIZE];
    unsigned char iv[AES_IV_SIZE];
...
std::vector<unsigned char> Encrypt::aes256_cbc_encrypt_with_iv(const std::vector<unsigned char>& plaintext,
                                                               std::string_view password,
                                                               const std::vector<unsigned char>& iv_in)
{
...
std::vector<unsigned char> Encrypt::aes256_cbc_decrypt_with_iv(const std::vector<unsigned char>& cipher,
                                                               std::string_view password,
                                                               const std::vector<unsigned char>& iv_in)
{
    if (iv_in.size() != AES_IV_SIZE)
...
std::vector<unsigned char> Encrypt::des3_ecb_encrypt(const std::vector<unsigned char>& plaintext,
                                                     std::string_view password)
{
...
std::vector<unsigned char> Encrypt::des3_ecb_decrypt(const std::vector<unsigned char>& cipher,
                                                     std::string_view password)
{
    unsigned char key[DES3_KEY_SIZE];
    unsigned char iv[DES3_IV_SIZE];
...
std::vector<unsigned char> Encrypt::des3_cbc_encrypt_with_iv(const std::vector<unsigned char>& plaintext,
                                                             std::string_view password,
                                                             const std::vector<unsigned char>& iv_in)
{
    if (iv_in.size() != DES3_IV_SIZE)
        throw std::invalid_argument("IV must be " + std::to_string(DES3_IV_SIZE) + " bytes for 3DES-CBC");
    unsigned char key[DES3_KEY_SIZE];
    unsigned char iv[DES3_IV_SIZE];
    derive_3des_key_iv(password, key, iv);
...
std::vector<unsigned char> Encrypt::des3_cbc_decrypt_with_iv(const std::vector<unsigned char>& cipher,
                                                             std::string_view password,
                                                             const std::vector<unsigned char>& iv_in)
{
    if (iv_in.size() != 8)
        throw std::invalid_argument("IV must be 8 bytes for 3DES-CBC");
...
std::vector<unsigned char> Encrypt::rsa_public_encrypt_pem(std::string_view public_pem,
...
std::pair<std::string, std::string> Encrypt::rsa_generate_keypair_pem(int bits)
{
    ...
    if (!pctx)
        throw RuntimeError("EVP_PKEY_CTX_new_id failed");
    if (EVP_PKEY_keygen_init(pctx) <= 0)
    {
        EVP_PKEY_CTX_free(pctx);
        throw RuntimeError("EVP_PKEY_keygen_init failed");
    }
    if (EVP_PKEY_CTX_set_rsa_keygen_bits(pctx, bits) <= 0)
    {
        EVP_PKEY_CTX_free(pctx);
        throw RuntimeError("EVP_PKEY_CTX_set_rsa_keygen_bits failed");
    }
    if (EVP_PKEY_keygen(pctx, &pkey) <= 0)
    {
        EVP_PKEY_CTX_free(pctx);
        throw RuntimeError("EVP_PKEY_keygen failed");
    }
    EVP_PKEY_CTX_free(pctx);
    ...
    if (!bio_priv || !bio_pub)
    {
        ...
        if (bio_pub)
            BIO_free(bio_pub);
        EVP_PKEY_free(pkey);
        throw RuntimeError("BIO_new failed");
    }
    ...
    if (!PEM_write_bio_PrivateKey(bio_priv, pkey, nullptr, nullptr, 0, nullptr, nullptr))
    {
        ...
        BIO_free(bio_pub);
        EVP_PKEY_free(pkey);
        throw RuntimeError("PEM_write_bio_PrivateKey failed");
    }
    if (!PEM_write_bio_PUBKEY(bio_pub, pkey))
    {
        ...
        BIO_free(bio_pub);
        EVP_PKEY_free(pkey);
        throw RuntimeError("PEM_write_bio_PUBKEY failed");
    }
...
std::vector<unsigned char> Encrypt::rsa_private_decrypt_pem(std::string_view private_pem,
