...
BOOST_AUTO_TEST_CASE(static_memory_pool_std_smart_ptr)
{
    ...
    vec.emplace_back("Hello");
    vec.emplace_back("World");
    std::shared_ptr<std::string> sp1 = std::allocate_shared<std::string>(
        HsBa::Slicer::StaticMemoryPool<std::string, 1024 * sizeof(std::string)>(), "Shared Hello");
    std::shared_ptr<std::string> sp2 = std::allocate_shared<std::string>(
        HsBa::Slicer::StaticMemoryPool<std::string, 1024 * sizeof(std::string)>(), "Shared World");
    BOOST_CHECK_EQUAL(*sp1, "Shared Hello");
    BOOST_CHECK_EQUAL(*sp2, "Shared World");
}
