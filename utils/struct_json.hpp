...
template <Aggregte T>
requires(!Reflectable<T>) void to_json_impl(const T& value, rapidjson::Value& json,
                                            rapidjson::Document::AllocatorType& allocator);

template <Aggregte T>
requires(!Reflectable<T>) void from_json_impl(const rapidjson::Value& json, T& value);
...
void add_member(rapidjson::Value& parent, std::string_view name, const FieldType& field,
                rapidjson::Document::AllocatorType& allocator);
...
void add_reflectable_members_impl(rapidjson::Value& parent, const FieldType& field,
                                  rapidjson::Document::AllocatorType& allocator, std::index_sequence<Is...>)
{
    if constexpr (sizeof...(Is) > 0)
    {
        int dummy[] = {((add_member(parent, StaticReflect::Reflector<FieldType>::template FieldName<Is>(),
                                    StaticReflect::Reflector<FieldType>::template GetField<Is>(field), allocator),
                         0))...};
        (void)dummy;
    }
}
...
void set_reflectable_fields_impl(const rapidjson::Value& json, T& value, std::index_sequence<Is...>)
{
    if constexpr (sizeof...(Is) > 0)
    {
        int dummy[] = {((json.HasMember(StaticReflect::Reflector<T>::template FieldName<Is>().data())
                             ? (set_field_from_json(StaticReflect::Reflector<T>::template GetField<Is>(value),
                                                    json[StaticReflect::Reflector<T>::template FieldName<Is>().data()]),
                                0)
                             : 0),
                        0)...};
        (void)dummy;
...
template <typename FieldType>
requires std::is_enum_v<FieldType> void add_member(rapidjson::Value& parent, std::string_view name,
...
requires(!Reflectable<FieldType>) void add_member(rapidjson::Value& parent, std::string_view name,
                                                  const FieldType& field, rapidjson::Document::AllocatorType& allocator)
{
    rapidjson::Value field_json(rapidjson::kObjectType);
...
requires(std::is_same_v<FieldType, std::string> ||
         std::is_same_v<FieldType, std::string_view>) void add_member(rapidjson::Value& parent, std::string_view name,
                                                                      const FieldType& field,
                                                                      rapidjson::Document::AllocatorType& allocator)
{
    parent.AddMember(rapidjson::StringRef(name.data()),
                     rapidjson::Value(field.data(), static_cast<rapidjson::SizeType>(field.size()), allocator),
                     allocator);
}

template <typename FieldType>
requires std::is_arithmetic_v<FieldType> void add_member(rapidjson::Value& parent, std::string_view name,
                                                         const FieldType& field,
                                                         rapidjson::Document::AllocatorType& allocator)
{
...
requires(!std::is_same_v<Range, std::string> && !std::is_same_v<Range, std::string_view>) void add_member(
    rapidjson::Value& parent, std::string_view name, const Range& rng, rapidjson::Document::AllocatorType& allocator)
{
...
void add_member(rapidjson::Value& parent, std::string_view name, const Opt& opt,
                rapidjson::Document::AllocatorType& allocator)
{
    ...
    if (opt.has_value())
    {
        ...
        else if constexpr (std::is_enum_v<ValueType>)
        {
            ...
            parent.AddMember(rapidjson::StringRef(name.data()), rapidjson::Value(enum_str.c_str(), allocator),
                             allocator);
        }
        else
        {
...
template <typename FieldType>
requires std::is_enum_v<FieldType> void set_field_from_json(FieldType& field, const rapidjson::Value& field_json)
{
...
template <Aggregte FieldType>
requires(!Reflectable<FieldType>) void set_field_from_json(FieldType& field, const rapidjson::Value& field_json)
{
...
template <typename FieldType>
requires std::is_same_v<FieldType, std::string> void set_field_from_json(FieldType& field,
                                                                         const rapidjson::Value& field_json)
{
    if (field_json.IsString())
...
template <typename FieldType>
requires std::is_arithmetic_v<FieldType> void set_field_from_json(FieldType& field, const rapidjson::Value& field_json)
{
    if (field_json.IsNumber())
        field = field_json.template Get<FieldType>();
}

template <std::ranges::range Range>
requires(!std::is_same_v<Range, std::string>) void set_field_from_json(Range& field, const rapidjson::Value& array_json)
{
    ...
    for (const auto& item_json : array_json.GetArray())
    {
        ...
        else if constexpr (std::is_arithmetic_v<ItemType>)
        {
            if (item_json.IsNumber())
                field.push_back(item_json.template Get<ItemType>());
        }
        else
        {
...
void set_field_from_json(Opt& opt, const rapidjson::Value& field_json)
{
    ...
    {
        if (field_json.IsString())
            opt = field_json.GetString();
    }
    else if constexpr (std::is_arithmetic_v<ValueType>)
    {
        if (field_json.IsNumber())
            opt = field_json.template Get<ValueType>();
    }
    else if constexpr (std::is_enum_v<ValueType>)
    {
        if (field_json.IsString())
            opt = EnumFromName<ValueType>(field_json.GetString());
    }
    else
    {
