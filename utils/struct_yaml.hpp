...
namespace HsBa::Slicer::Utils
{
	...
	template<typename T>
	concept YAMLValueConvertible =
		requires(const T & value, YAML::Node & node) {
	...
	namespace detail
	{
		...
		void add_reflectable_members_impl(YAML::Node& parent, const FieldType& field, std::index_sequence<Is...>)
		{
			...
			{
				int dummy[] = { ((add_member(parent, StaticReflect::Reflector<FieldType>::template FieldName<Is>(), StaticReflect::Reflector<FieldType>::template GetField<Is>(field)), 0))... };
				(void)dummy;
			}
		}
		...
		void add_member(YAML::Node& parent, std::string_view name, const Range& rng)
		{
		...
		void to_yaml_impl(const T& value, YAML::Node& node)
		{
			...
			boost::pfr::for_each_field(value, [&](const auto& field, size_t idx) {
				add_member(node, field_names[idx], field);
			});
		}
		...
		void set_field_from_yaml(FieldType& field, const YAML::Node& field_node)
		{
			...
			{
				if (field_node.IsScalar()) field = field_node.as<std::string>();
			}
			else if constexpr (std::is_arithmetic_v<FieldType>)
			{
				if (field_node.IsScalar()) field = field_node.as<FieldType>();
			}
			else
			{
		...
		template<std::ranges::range Range>
		requires (!std::is_same_v<Range, std::string>)
		void set_field_from_yaml(Range& field, const YAML::Node& array_node)
		{
			using ItemType = std::remove_cvref_t<typename Range::value_type>;
			field.clear();
			if (!array_node.IsSequence()) return;
			...
			if constexpr (sizeof...(Is) > 0)
			{
				int dummy[] = {
					((node[StaticReflect::Reflector<T>::template FieldName<Is>().data()]
						? (set_field_from_yaml(StaticReflect::Reflector<T>::template GetField<Is>(value), node[StaticReflect::Reflector<T>::template FieldName<Is>().data()]), 0)
						: 0), 0)...
				};
				(void)dummy;
			}
