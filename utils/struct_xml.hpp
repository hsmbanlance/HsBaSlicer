...
namespace HsBa::Slicer::Utils
{
	...
	template<typename T>
	concept TinyXmlConvertible =
		requires(const T & value, tinyxml2::XMLElement * element, tinyxml2::XMLDocument * doc) {
	...
	namespace detail
	{
		...
		void add_element(tinyxml2::XMLElement* parent, std::string_view name, const FieldType& field, tinyxml2::XMLDocument* doc);

		// Helper: expand reflectable fields into add_element calls
		template<typename FieldType, std::size_t... Is>
		void add_reflectable_elements_impl(tinyxml2::XMLElement* parent, const FieldType& field, tinyxml2::XMLDocument* doc, std::index_sequence<Is...>)
		{
			if constexpr (sizeof...(Is) > 0)
			{
				int dummy[] = { ((add_element(parent, StaticReflect::Reflector<FieldType>::template FieldName<Is>(), StaticReflect::Reflector<FieldType>::template GetField<Is>(field), doc), 0))... };
				(void)dummy;
			}
		}
		...
		void set_reflectable_fields_impl(const tinyxml2::XMLElement* element, T& value, std::index_sequence<Is...>)
		{
			...
			{
				int dummy[] = {
					((element->FirstChildElement(StaticReflect::Reflector<T>::template FieldName<Is>().data())
						? (set_field_from_xml(StaticReflect::Reflector<T>::template GetField<Is>(value), element->FirstChildElement(StaticReflect::Reflector<T>::template FieldName<Is>().data())), 0)
						: 0), 0)...
				};
				(void)dummy;
			}
		...
		void add_element(tinyxml2::XMLElement* parent, std::string_view name, const Range& rng, tinyxml2::XMLDocument* doc)
		{
		...
		template<std::ranges::range Range>
			requires (!std::is_same_v<Range, std::string>)
		void set_field_from_xml(Range& field, const tinyxml2::XMLElement* array_element)
		{
			using ItemType = std::remove_cvref_t<typename Range::value_type>;
			field.clear();
			for (const tinyxml2::XMLElement* item_element = array_element->FirstChildElement("item");
				 item_element;
		...
		void set_field_from_xml(Opt& opt, const tinyxml2::XMLElement* field_element)
		{
			...
			{
				const char* text = field_element->GetText();
				if (text) opt = EnumFromName<ValueType>(text);
			}
			else
			{
	...
	template<std::ranges::range Range>
		requires (!std::is_same_v<Range, std::string>)
	std::unique_ptr<tinyxml2::XMLDocument> to_xml(const Range& rng, const char* root_name = "root")
	{
		auto doc = std::make_unique<tinyxml2::XMLDocument>();
		tinyxml2::XMLElement* root = doc->NewElement(root_name);
		doc->InsertFirstChild(root);
	...
	template<std::ranges::range Range>
		requires (!std::is_same_v<Range, std::string>)
	Range from_xml(const tinyxml2::XMLDocument& doc)
	{
		const tinyxml2::XMLElement* root = doc.RootElement();
		if (!root)
		{
